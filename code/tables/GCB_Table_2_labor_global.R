...
cheadername <- names(t)
cnewname <- c(" ", paste0("≤ ", cutoffVals)) # space needed in the name for the first column
cvalues <- setNames(cnewname, cheadername)

t_flex <- flextable(t) |> 
  set_header_labels(values = cvalues) |>
  add_header_row(colwidths = c(1,5),
                 values = c("Emission scenario & period", "PWC reduction"), top = TRUE) |> 
...
t_flex
save_as_docx(t_flex, values = NULL, path = "tables/table2_stressedLaborCts_global.docx")
